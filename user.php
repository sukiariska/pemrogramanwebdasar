...
<div class="col-lg-9 mt-2">
    <div class="card">
        <div class="card-header ">
            Selamat Datang, Di Halaman User SuRider
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col d-flex justify-content-end">
                    <button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#ModalTambahUser"> Tambah User</button>
                </div>
            </div>
...
<div class="modal-footer">
        ...
        <button type="submit" class="btn btn-danger" name="input_user_validate" value="12345">Save changes</button>
        </div>
</form>
        </div>
    ...
    <div class="row">
                    <div class="col-lg-4">
                    <div class="form-floating mb-3">
                    <select disabled class="form-select" arial-label="Defaul select example " required  name="level" id=""> 
                    <?php
                    $data = array("Owner/Admin","Karyawan","Costumer","Sistem");
...
<!-- Akhir Modal view-->
    ...
    <div class="invalid-feedback">
        ...
        <div class="col-lg-6">
    <div class="form-floating mb-3">
    <input <?php echo ($row['username'] == $_SESSION['username_rental_mobil']) ? 'disabled' : '' ; ?> type="email" class="form-control" id="floatingInput" placeholder="name@example.com" name="username" required value="<?php echo $row['username'] ?>">
    <label for="floatingInput">Username</label>
    <div class="invalid-feedback">
        Masukkan Username .
        </div>
    ...
    <div class="row">
                    <div class="col-lg-4">
                    <div class="form-floating mb-3">
                        <select class="form-select" arial-label="Defaul select example " required  name="level" id=""> 
                    <?php
                    $data = array("Owner/Admin","Karyawan","Costumer","Sistem");
                    foreach($data as $key => $value){
                        if($row['level'] ==$key+1){
        ...
        <div class="col-lg-8">
    <div class="form-floating mb-3" >
    <input  type="number" class="form-control" id="floatingInput" placeholder="08xxxxx" name="NoTelepon"  value="<?php echo $row['NoTelepon'] ?>">
    <label for="floatingInput">NoTelepon</label>
    </div>
    </div>
    </div>
...
<div class="modal fade" id="ModalDelete<?php echo $row['id']?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    <div class="modal-content">
        ...
        <div class="col-lg-12">
            <?php
            if($row['username'] == $_SESSION['username_rental_mobil']){
...
<div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
        <button type="submit" class="btn btn-danger" name="input_user_validate" value="12345" <?php echo ($row['username'] == $_SESSION['username_rental_mobil']) ? 'disabled' : '' ; ?> >Hapus</button>
        </div>
</form>
        </div>
    </div>
    ...
    <div class="modal-content">
        ...
        <div class="col-lg-12">
            ...
            if($row['username'] == $_SESSION['username_rental_mobil']){
                ...
                echo "Apakah anda yakin ingin  mengreset password <b> $row[username]</b> menjadi paswword bawaan sistem yaitu <b>password</b>";
            }
            ?>
...
<div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
        <button type="submit" class="btn btn-success" name="input_user_validate" value="12345" <?php echo ($row['username'] == $_SESSION['username_rental_mobil']) ? 'disabled' : '' ; ?> > reset password</button>
        </div>
</form>
        </div>
    </div>
